...
def GL_Request_pdf(soap):
    logger.info("Requesting to GL Interface")
    url = f"{oracle_url}/fscmRestApi/resources/11.13.18.05/erpintegrations"
    headerss = {"Content-Type": "application/pdf",
                "Connection": "Keep-Alive"
                }
    user,passw = read_credentials()
    return requests.post(url, data=soap, auth=(user, passw), headers=headerss)
...
"ESSParameters":"{Ledger_Id},300000001414463,3002,{Ledger_Id},USD,{Period_Name},A,#NULL,#NULL,#NULL,#NULL,#NULL,Y,300000001412001,3002X300000001412001",
"ReqstId":null
}}'''
...
def Do_MPA(ledger_id, Year, Month):
    ...
    if PID != -1:
        ...
        with open(f"{data_path}\\PID_{ledger_id}_{mo}_{year_two_digits}.INFO","a+") as INFO_file:
                INFO_file.write(f"MPA Child PID :::::  {pid_to_download}\n")
        while True:
...
def create_report(CPE_zipfile,pid,ent):
    ...
    try:
        ...
        new_data={}
        count=0
        if type(data['CLASS_S']) is dict:
            new_data[count]= data['CLASS_S']
            count+=1
        elif type(data['CLASS_S']) is list:
...
def create_report(CPE_zipfile,pid):
    ...
    #try:
        ...
        print(data)
        for i in data:
            if type(i['CLASS_S']) is dict:
                new_data[count]= i['CLASS_S']
                count+=1
            elif type(i['CLASS_S']) is list:
                for j in range(len(i['CLASS_S'])):
                    new_data[count]= i['CLASS_S'][j]
                    count+=1
    ...
    if "No" not in report:
        ...
        with open(f"{config_path}\\revid.txt","r") as rev_data:
            ...
            for i in revent:
                ...
                rev_name=i.split(",")[1]
                for j in revids:
                    if rev_name in j and "SL" not in j:
