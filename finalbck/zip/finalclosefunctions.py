    ...
    try:
        ...
        for i in region_ledgers:
            ...
            if len(response.decode().strip().splitlines()) != valid_length:
                break
                return f"Python Error : Ledger  {ledger} of {Entity} is not posted"
        return 0
    except:
...
def Revaluation(ledger_id, Year, Month):
    ...
    if "SL" not in rev_name:
        ...
        if int(PID) != -1:
            ...
            zipdata = get_files(PID, "OUT")
            zipfile_name = f"{zip_path}\\rev_{ledger_id}_{Year}_{Month}.zip"
            with open(f"{zipfile_name}", "wb") as zip:
                zip.write(zipdata)
            if Fail_Flag == 1:
                with open(f"{zip_path}\\rev_error-{ledger}_{ledger_id}.txt", "w") as errortxt:
                    errortxt.write(f"Error or Warning on Revaluation for PID: {PID}")
                return f"{zip_path}\\rev_error-{ledger}_{ledger_id}.txt"
            else:
                with open(f"{data_path}\\rev_{ledger_id}{Year}{Month}.cfg","a+") as rev_file:
                    rev_file.write(f"{ledger_id}{Year}{Month}\n")
                return f"{zipfile_name}"
        else:
            with open(f"{zip_path}\\rev_error-{ledger}_{ledger_id}.txt", "w") as errortxt:
                errortxt.write(f"Can't get valid id revaluation for {ledger}")
            return f"{zip_path}\\rev_error-{ledger}_{ledger_id}.txt"
...
def Do_MPA(ledger_id, Year, Month):
    ...
    child_filename=get_mpa_report(child_zipfile_name)
    #############
...
def DO_CPE(ledger_id, period, Year, Month):
    ...
    while True:
        ...
        if Status == "SUCCEEDED" or Status == "ERROR" or Status == "WARNING":
            ...
            break
    zipdata = get_files(PID, "All")
    zipfile_name = f"{zip_path}\\cpe_{ledger_id}_{PID}.zip"
    with open(f"{zipfile_name}", "wb") as zip:
        zip.write(zipdata)
    return f"{zipfile_name}"
...
def Do_Translation(ledger_id, Period):
    ...
    try:
        with open(f"{data_path}\\tra_{ledger_id}{Year}{Month}.cfg","r") as tra_file:
            return f"{zip_path}\\tra_{ledger_id}_{Year}_{Month}.zip"
    except:
        pass
    Fail_Flag = 0
    try:
        ...
        if int(PID) != -1:
            while True:
                ...
                if Status == "SUCCEEDED" or Status == "ERROR" or Status == "WARNING":
                    ...
                    break
            zipdata = get_files(PID, "All")
            zipfile_name = f"{zip_path}\\trans_{ledger_id}_{Year}_{Month}.zip"
            with open(f"{zipfile_name}", "wb") as zip:
                zip.write(zipdata)
        else:
...
def get_mpa_report(file):
    ...
    #try:
        ...
        write_pdf(data_lines,f"{unzip_path}\\mpa_report_{pid}.pdf")
        return f"{unzip_path}\\mpa_report_{pid}.pdf"
    #except:
...
def Get_L_Status(Period,ledger_id,name):
    ...
    return f"Status for ledger {name} on period {Period} : {status}"
